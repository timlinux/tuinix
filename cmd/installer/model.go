...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "q":
			...
			switch m.state {
			case stateSplash, stateNetworkCheck, stateDisk, stateDiskMulti, stateLocale, stateKeymap, statePackageSet, stateSSH, stateSummary, stateStorageMode, stateComplete, stateError:
				return m, tea.Quit
			}
		case "enter":
		...
		case "up", "k":
			if m.state == stateDisk || m.state == stateDiskMulti || m.state == stateLocale || m.state == stateKeymap || m.state == statePackageSet || m.state == stateSSH || m.state == stateStorageMode {
				if m.selectedIdx > 0 {
					m.selectedIdx--
				}
			}

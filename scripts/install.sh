...
collect_user_input() {
  gum style --foreground="#5277C3" "📝 User Account Setup"
  echo ""

  # Username
  USERNAME=$(gum input --placeholder="Enter username (e.g., john, alice)" --prompt="Username: ")
  while [[ -z "$USERNAME" || ! "$USERNAME" =~ ^[a-z_][a-z0-9_-]*$ ]]; do
    gum style --foreground="#DC3545" "Invalid username. Use lowercase letters, numbers, underscores, hyphens. Must start with letter or underscore."
    USERNAME=$(gum input --placeholder="Enter username (e.g., john, alice)" --prompt="Username: ")
  done

  # Full name (for git and system)
  USER_FULLNAME=$(gum input --placeholder="Enter your full name (e.g., John Smith)" --prompt="Full name: ")
  while [[ -z "$USER_FULLNAME" ]]; do
    gum style --foreground="#DC3545" "Full name is required."
    USER_FULLNAME=$(gum input --placeholder="Enter your full name (e.g., John Smith)" --prompt="Full name: ")
  done

  # Email (for git)
  USER_EMAIL=$(gum input --placeholder="Enter your email (e.g., john@example.com)" --prompt="Email: ")
  while [[ -z "$USER_EMAIL" || ! "$USER_EMAIL" =~ ^[^@]+@[^@]+\.[^@]+$ ]]; do
    gum style --foreground="#DC3545" "Please enter a valid email address."
    USER_EMAIL=$(gum input --placeholder="Enter your email (e.g., john@example.com)" --prompt="Email: ")
  done

  gum style --foreground="#28A745" "✅ User: $USERNAME ($USER_FULLNAME <$USER_EMAIL>)"
...
generate_host_config() {
  ...
  # Generate the user-specific nix file
  local users_dir="$PROJECT_ROOT/users"
  cat >"$users_dir/$USERNAME.nix" <<EOF
...
EOF
  ...
  gum style --foreground="#28A745" "✅ User configuration generated: users/$USERNAME.nix"
...
setup_user_flake() {
  ...
  local user_dir="/mnt/home/$USERNAME/tuinix"
  local host_dir="$PROJECT_ROOT/hosts/$HOSTNAME"
  local users_dir="$PROJECT_ROOT/users"
  ...
  # Copy the generated user configuration into the clone
  gum style --foreground="#FFC107" "Adding user configuration for '$USERNAME'..."
  cp "$users_dir/$USERNAME.nix" "$user_dir/users/$USERNAME.nix"

  # Configure git identity for the commit (required on fresh systems)
  git -C "$user_dir" config user.name "$USER_FULLNAME"
  git -C "$user_dir" config user.email "$USER_EMAIL"

  # Register the new host and user config in git and commit
  git -C "$user_dir" add "hosts/$HOSTNAME" "users/$USERNAME.nix"
  git -C "$user_dir" commit -m "Add host and user configuration for $HOSTNAME

Generated by tuinix installer on $(date -u '+%Y-%m-%d %H:%M:%S UTC')
Host: $HOSTNAME
User: $USERNAME ($USER_FULLNAME <$USER_EMAIL>)
Host ID: $HOST_ID
Disk: $DISK
Locale: $LOCALE
Keymap: $KEYMAP"

  # Get the user's uid/gid (will be created by nixos-install)
  # Default to 1000 for the first user
  local user_uid=1000
  local user_gid=100
  ...
  nixos-enter --root /mnt --command "ln -sf /home/$USERNAME/tuinix /etc/tuinix-user"

  gum style --foreground="#28A745" "✅ User flake ready at /home/$USERNAME/tuinix"
  gum style --foreground="#654321" \
    "  • Upstream: $repo_url" \
    "  • Host config committed: hosts/$HOSTNAME/" \
    "  • User config committed: users/$USERNAME.nix" \
    "  • Rebuild with: sudo nixos-rebuild switch --flake /home/$USERNAME/tuinix#$HOSTNAME"
}
...
complete_installation() {
  gum style \
    ...
    "" \
    "Your personal flake is at: /home/$USERNAME/tuinix" \
    "  Rebuild: sudo nixos-rebuild switch --flake /home/$USERNAME/tuinix#$HOSTNAME" \
